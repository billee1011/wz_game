...
//        }
    ...
    private void updatePlayerCoin(Player player, int coin, boolean add, int db, int roomId, boolean isPersonal) {
        // 此处从logic服同步需要加减的金币,必须先同步金币，在退出房间
        // 判断是否是私房
        if (isPersonal) {
            logger.info("私房 {}操作金币 ", roomId);
            if (add) {
                if (coin > 0 && CenterServer.getInst().isRankReward()) {
                    player.setWinScore(db * (player.getWinScore() + coin / 100));
                }
                player.setCoin(player.getCoin() + coin);
                logger.info("金币更新 :玩家 {} Logic同步金币， 金币增加 {}，当前总金币数为 {}", player.getPlayerId(), coin, player.getCoin());
            } else {
                player.setCoin(player.getCoin() - coin);
                logger.info("金币更新 :玩家 {} Logic同步金币， 金币减少 {}，当前总金币数为 {}", player.getPlayerId(), coin, player.getCoin());
            }
            return;
        }
        ...
        if (room == null) {
            logger.error("player {} updatePlayerCoin error , roomId is {}", player.getPlayerId(), roomId);
        }
        if (2 == room.getClassify()) {
            if (add) {
                player.setTiyanCoin(player.getTiyanCoin() + coin);
            } else {
                player.setTiyanCoin(player.getTiyanCoin() - coin);
            }
        } else {
...
//		}
    ...
    private void actionDispatchGate(ChannelHandlerContext ioSession, CocoPacket srcPacket, AbstractHandlers.MessageHolder<MessageLite> message) {
        String gateIp = null;
        int gatePort = 0;
        if (CenterServer.getInst().isUseSlb()) {//是否使用slb硬件ip
            ...
            List<String> slbList = null;
            while (sessionList.size() > 0) {
                session = sessionList.remove(0);
                slbList = CenterServer.getInst().getGate2Slb().get(session.getRemoteAddress());
                if (slbList != null && slbList.size() != 0) {
                    break;
                }
                logger.warn("{} gate未配置slb,使用下一个gate", session.getRemoteAddress());
            }
            if (session == null) {
            ...
            if (slbList == null || slbList.size() == 0) {
                logger.warn("{} 重试最后一个gate也未配置slb,用户无法进入游戏", session.getRemoteAddress());
                return;
            }

            gateIp = slbList.get(Randomizer.nextInt(slbList.size()));
            gatePort = session.getLocalPort();
        } else {
            ...
            gateIp = session.getRemoteAddress();
            gatePort = session.getLocalPort();
        }
